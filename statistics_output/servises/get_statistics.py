...
from receipt_load.models import Store_receipt
...
def get_month_statistics(year, month):
    ...
    for category in Product_category.objects.all():
        ...
        statistics[category.categoryName] = category_statistics
...
def itemization_month_category(year, month, category):
    purchases = filter_by_date(Purchase.objects.all(), year, month)
    category = Product_category.objects.get(categoryName=category)
...
def filter_by_category(purchases, category):
    name_by_category = Products.objects.filter(category=category)
...
def filter_by_date(purchases, year, month):
    receipt_month = Store_receipt.objects.filter(
        dateTime__year=year,
